  ...
  return (
    <div className="flex flex-col gap-3">
      {sections.map(section => (
        <div
          key={section.title}
          className="bg-white border border-[var(--brand-line)] rounded-[10px] p-4 shadow-[0_2px_12px_rgba(151,155,192,0.08)]"
        >
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[14px]">{section.icon}</span>
            <span className="text-[11px] font-bold text-[var(--brand-ink-0)]">{section.title}</span>
          </div>
          <p className="text-[11px] text-[var(--brand-ink-2)] leading-relaxed">{section.content}</p>
        </div>
      ))}
    </div>
  )
